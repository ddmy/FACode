...
// 收集帧动画路径所有图片信息
const redPathOriginDir = fs.readdirSync(path.join(__dirname, REDPATH))
// 将目录内合适的文件转成base64格式
const redPathDir = redPathOriginDir
  .filter(item => item.toLocaleUpperCase().endsWith(TYPE.toLocaleUpperCase()))
  .sort((a, b) => a.split('.')[0] - b.split('.')[0])
  .map(item => imgToBase64(path.join(__dirname, REDPATH + item)))
...
if (argv.mode.toLocaleUpperCase() === 'JS') {
  ...
  fs.writeFileSync(path.join(__dirname, WRITEPATH + ID + '.js'), jsTemp)
...
// CSS动画
  ...
  fs.writeFileSync(path.join(__dirname, WRITEPATH + ID + '.css'), cssTemp)
